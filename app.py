...
# Stack predictions as new feature inputs
stacked_features = np.column_stack((dt_preds, rf_preds))

# Train meta-model (XGBoost) on stacked predictions
model = XGBRegressor(n_estimators=100, learning_rate=0.1, max_depth=5, random_state=42)
model.fit(stacked_features, y_test)
...
def prediction():
    ...
    if request.method == 'POST':
        ...
        stacked_input = np.column_stack((dt_pred, rf_pred))
        predicted_yield = model.predict(stacked_input)
